...
const LoanCalculator = forwardRef<HTMLDivElement, LoanCalculatorProps>(
  ({ showLoginPopup }, ref) => {
    ...
    const calculateLoan = async (amount: number, period: number) => {
      try {
        ...
        const response = await axios.get<LoanCalcResponse>(
          `${baseURL}/api/loan-calc`,
          {
            params: { amount, period },
          },
        );
      ...
      for (let i = 12; i <= 36; i += 4) {
        options.push(
          <option
            key={i}
            value={i}
            data-testid={`ib-small-loan-calculator-button-period-option-${i - 12}`}
          >
            {i}
          </option>,
        );
      ...
      <Container ref={ref}>
        ...
        <div className="input-box">
          ...
          <select
            ...
            data-testid="ib-small-loan-calculator-field-period"
          >
            {renderOptions()}
          </select>
          <input
        ...
        <div className="monthly-payment">
          Monthly payment <br />{' '}
          <span data-testid="ib-small-loan-calculator-field-monthlyPayment">
            {monthlyPayment}
          </span>
        </div>
        {error && (
          <div
            className="error"
            data-testid="id-small-loan-calculator-field-error"
          >
            {error}
          </div>
        )}
        <button
          ...
          data-testid="id-small-loan-calculator-field-apply"
        >
          Apply Now
        </button>
      </Container>
